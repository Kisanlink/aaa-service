...
// @Param source header string false "Access source (e.g., 'admin/panel' for admin panel access)"
...
func (s *UserHandler) LoginRestApi(c *gin.Context) {
	...
	const (
		AdminPanelSource    = "admin/panel"
		RoleAdmin           = "ADMIN"
		RoleSuperAdmin      = "SUPER_ADMIN"
		RoleCustomerSupport = "CUSTOMER_SUPPORT"
	)

	// Then in your LoginRestApi function:
	source := c.GetHeader("source")
	if source == AdminPanelSource {
		allowedRoles := map[string]bool{
			RoleAdmin:           true,
			RoleSuperAdmin:      true,
			RoleCustomerSupport: true,
		}
		...
		if !hasPermission {
			helper.SendErrorResponse(c.Writer, http.StatusForbidden, []string{"Access Denied: Insufficient permission"})
			return
		}
	}
