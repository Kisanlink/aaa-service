...
func (s *Server) GetUserByMobileNumber(ctx context.Context, req *pb.GetUserByMobileNumberRequest) (*pb.GetUserByMobileNumberResponse, error) {
	mobile := req.GetMobileNumber()
	if mobile == 0 {
		return nil, status.Error(codes.InvalidArgument, "mobile number is required")
	}

	user, err := s.userService.FindUserByMobile(mobile)
	if err != nil {
	...
	return &pb.GetUserByMobileNumberResponse{
